  ...
  t = jQuery, a = {
    ...
    showProducts: function(s) {
      let r = t("<form>").attr({
        ...
        }).html(BX.message('PRICE_COUNT')).append(p),
        u = t("<div>").attr({
          class: "smart-basket__product-price"
        }).html(`${BX.message('PRICE')} / ${a.smartBasketCurrency}`).append(c).append(d);
      c.click(() => {
        d.fadeToggle()
      }), p.click(t => {
      ...
      let m = t("<div>").attr({
        ...
        }).html(`${BX.message('ITOGO_PRICE')} / ${a.smartBasketCurrency}`),
        k = t("<div>").attr({
          class: "smart-basket__product-delete"
        }).text(BX.message('DEL'));
      ...
      for (let n in s) {
        ...
        for (let a in s[n]) {
          ...
          if ("sbName" === a && (c.append(`<span>${s[n][a]}</span>`), p.val(s[n][a])), "sbSize" === a && void 0 !== s[n][a] && (c.children("span").append(`<br><span class="smart-basket__product-size"> ${s[n][a]}</span>`), d.val(s[n][a])), "sbPrice" === a && (_.html(s[n][a]), h.val(s[n][a])), "sbPriceCommon" === a) {
            let t = +s[n][a];
            y.val(+t.toFixed(2))
          }
    ...
    stateBasket: function() {
      ...
      } else console.log(BX.message('ERROR_ADMIN'));
      return e.getSmartBasketMinState(o), c.click(function() {
        let r = e.getProducts() || {},
          n = t(this).attr("data-sb-id-or-vendor-code");
        if (void 0 !== r[n]) {
          let a = t(this).html();
          return t(this).text(BX.message('INCARTALREADY_THIRD')), t("body").append(e.alertBlock("alreadyAdded")), setTimeout(() => {
            t(this).html(a)
          }, 1500), !1
        } {
          let i = {};
    ...
    commonResult: function(e, s, r) {
      let n = 0,
        ...
        }).text(BX.message('EMPTY')).fadeOut(),
        l = t("<div>").attr({
          class: "smart-basket__success-title"
        }) /*.text(BX.message('CONFIRM'))*/ .css("display", "none"),
        c = t("<div>").attr({
      ...
      if (0 == +n ? o.fadeIn() : o.fadeOut(), r) t("." + l.attr("class")).css("display", "block"), setTimeout(() => {
        ...
        if (!s) return d.html(`<span>${BX.message('ITOGO')} </span> ${n}`), p.html(`<span>${BX.message('ITOGO_COST')} </span> <input name="finalPrice" class="smart-basket__total-cost" type="text" value="${i.toFixed(2)} ${a.smartBasketCurrency}">`), c.append(o).append(l).append(d).append(p), c;
        0 == +n && t("." + o.attr("class")).css("display", "block"), t("." + d.attr("class")).html("<span>${BX.message('ITOGO')} </span> "), t("." + d.attr("class")).html(`<span>${BX.message('ITOGO')} </span> ${n}`), t("." + p.attr("class")).html("<span>${BX.message('ITOGO_COST')} </span> "), t("." + p.attr("class")).html(`<span>${BX.message('ITOGO_COST')} </span> <input name="finalPrice" class="smart-basket__total-cost" type="text" value="${i.toFixed(2)} ${a.smartBasketCurrency}">`)
      }
    },
    ...
    userForm: function() {
      ...
      if (c.append(p), a.agreement.isRequired) {
        ...
        r = a.agreement.isLink ? t("<a>").attr({
          ...
          class: "smart-basket__link-agreement"
        }), s.is(":checked") ? r.text(BX.message('AGREE')) : r.text(BX.message('DISAGREE')), e.click(function() {
          s.is(":checked") ? (r.text(BX.message('DISAGREE')), s.val(BX.message('DISAGREE')), m.attr({
            disabled: "disabled"
          }), d.addClass("smart-basket__input-wrapper_error")) : (r.text(BX.message('AGREE')), s.val(BX.message('AGREE')), m.attr({
            disabled: !1
    ...
    sendCart: function() {
      ...
      t.ajax({
        ...
        success: function(result) {
          if (result.status) {
            $(".smart-basket__success-title").html(result.message);
            if (result.link) {
              //setTimeout(window.top.location=result.link);
        ...
        complete: function(s) {
          ...
          let r = e.getProducts() || {};
          e.commonResult(r, "", "submitEvent"), localStorage.clear(), "" !== a.yaCounter.counterID && "" !== a.yaCounter.targetID && ym(a.yaCounter.counterID, "reachGoal", a.yaCounter.targetID),
    ...
    return e[a] ? e[a].apply(this, Array.prototype.slice.call(arguments, 1)) : "object" != typeof a && a ? void t.error(BX.message('METHOD') + a + BX.message('NOTFOUND')) : e.init.apply(this, arguments)
  }
});
